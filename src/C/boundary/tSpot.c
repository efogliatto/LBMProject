...
void tSpot( struct bdInfo* bdElements, struct lbeField* field, struct latticeInfo* lattice, struct macroFields* mfields, int** nb, int fid, int bndId ) {
	...
	for( k = 0 ; k < lattice->Q ; k++ ) {
		...
		if( nbid != -1 ) {
			...
			if( i == (int)(bdElements->_nbel[bndId]/2) ) {  eq_bnd = mfields->Cv * bdElements->_value[fid][bndId][1] * f_eq_nb[k]; }
			else {  eq_bnd = mfields->Cv * bdElements->_value[fid][bndId][0] * f_eq_nb[k]; }
