...
extern "C" {
    ...
    string ddx = mp.lookUpEntry<string>("directionalDerivatives/type");
    info.lattice.dd = new char[ddx.length() + 1];
    std::strcpy(info.lattice.dd, ddx.c_str());
		  
    // info.lattice.ddx = c_hash( ddx.c_str() );
