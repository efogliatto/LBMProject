...
// Default constructor
periodicBox::periodicBox() : box() {}

// Default constructor
periodicBox::periodicBox(const string& name) : box(name) {}
...
// Reading properties
void periodicBox::readProperties(const std::string& dictName) {
...
// Mesh construction
const vector<Vector3> periodicBox::points() const {
...
const vector< vector<int> > periodicBox::neighbours(const basicLBModel* lbm, const vector<Vector3>& points) const {
    ...
    // Reassign neighbours in periodic Y faces
    if(_ny > 1) {
	...
	for(uint i = 0 ; i < _nx ; i++) {
		...
		// Periodic indices
		uint idy0 = i +  k*_nx*_ny,
		    idy1 = i + (_ny-1)*_nx + k*_nx*_ny;

		// Assign missing neighbours
		for(uint velId = 0 ; velId < lbm->Q() ; velId++) {
		    
		    if(  neigh[idy0][velId] == -1  ) {
			neigh[idy0][velId] = neigh[idy1][velId];
		    }

		    if(  neigh[idy1][velId] == -1  ) {
			neigh[idy1][velId] = neigh[idy0][velId];
		    }
    ...
    // Reassign neighbours in periodic Z faces
    if(_nz > 1) {
    ...
    // // p0, p7
    // cornerPairs[0] = std::make_pair(0, _nx * _ny * _nz - 1);

    // // p1, p6
    // cornerPairs[1] = std::make_pair( _nx - 1, _nx * _ny * _nz - _nx );
    
    // // p2, p5
    // cornerPairs[2] = std::make_pair( _nx * (_ny-1), _nx * _ny * (_nz-1) + _nx - 1);

    // // p3, p4
    // cornerPairs[3] = std::make_pair( _nx * _ny - 1, _nx * _ny * (_nz-1) );
...
// Boundary map
const map<string, vector<uint>> periodicBox::boundaryMap() const {
	...
	for(uint i = 0 ; i < _nx ; i++) {
	    for(uint j = 0 ; j < 1 ; j++) {
		for(uint k = 0 ; k < _nz ; k++) {
		    bmap["periodicBox_Y0"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	}
	...
	for(uint i = 0 ; i < _nx ; i++) {
		...
		for(uint k = 0 ; k < _nz ; k++) {
		    bmap["periodicBox_Y1"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	}    
	...
	for(uint i = 0 ; i < 1 ; i++) {
	    for(uint j = 1 ; j < (_ny-1) ; j++) {
		for(uint k = 0 ; k < _nz ; k++) {
		    bmap["periodicBox_X0"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	}
	...
	for(uint i = (_nx -1) ; i < _nx ; i++) {
	    for(uint j = 1 ; j < (_ny-1) ; j++) {
		for(uint k = 0 ; k < _nz ; k++) {
		    bmap["periodicBox_X1"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	} 
	...
	for(uint i = 1 ; i < (_nx-1) ; i++) {
	    for(uint j = 1 ; j < (_ny-1) ; j++) {
		for(uint k = 0 ; k < 1 ; k++) {
		    bmap["periodicBox_Z0"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	}
	...
	for(uint i = 1 ; i < (_nx-1) ; i++) {
		...
		for(uint k = (_nz-1) ; k < _nz ; k++) {
		    bmap["periodicBox_Z1"].push_back( i + j*_nx + k*_nx*_ny );
		}	    
	    }
	}
...
// VTK Cells
const vector< vector<uint> > periodicBox::VTKCells(const basicLBModel* lbm, const vector<Vector3>& points, const vector< vector<int> >& nbr) const {
